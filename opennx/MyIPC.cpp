...
MyIPC::OnOutReceived(wxCommandEvent &event)
{
    ...
    switch (m_eType) {
        case TypeNone:
            ::myLogTrace(MYTRACETAG, wxT("process O: '%s'"), msg.c_str().AsChar());
            if (m_pEvtHandler) {
                wxCommandEvent upevent(wxEVT_GENERIC, wxID_ANY);
        ...
        case TypeSsh:
            ...
            if (msg.StartsWith(wxT("FREENX>")))
                    msg = msg.Mid(4);
            code = parseCode(msg);
            ::myLogTrace(MYTRACETAG, wxT("nxssh O[%04d]: '%s'"), code, msg.c_str().AsChar());
            if (m_pEvtHandler) {
                wxCommandEvent upevent(wxEVT_NXSSH, wxID_ANY);
...
MyIPC::OnErrReceived(wxCommandEvent &event)
{
    ...
    switch (m_eType) {
        case TypeNone:
            ::myLogTrace(MYTRACETAG, wxT("process E: '%s'"), msg.c_str().AsChar());
            if (m_pEvtHandler) {
                wxCommandEvent upevent(wxEVT_GENERIC, wxID_ANY);
        ...
        case TypeSsh:
            code = parseCode(msg);
            ::myLogTrace(MYTRACETAG, wxT("nxssh E[%04d]: '%s'"), code, msg.c_str().AsChar());
            if (m_pEvtHandler) {
                wxCommandEvent upevent(wxEVT_NXSSH, wxID_ANY);
                upevent.SetInt(ActionLog);
                upevent.SetString(event.GetString());
